...
const BubbleScene: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const setup = () => {
      ...
      const updateVertices = (time: number): void => {
        // ノイズ量
      ...
      const onScroll = (): void => {
        ...
        if (scrollY >= startScroll && scrollY <= endScroll) {
          ...
          animate(bubbleRef.current!.position, {
            x: moveX,
            duration: 500,
            ease: 'outQuad',
          });
  ...
  return (
    <canvas ref={canvasRef} className="fixed left-0 top-0 h-screen w-full" />
  );
};
